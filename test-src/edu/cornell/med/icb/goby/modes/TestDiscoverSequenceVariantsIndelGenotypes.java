 ...
 *         Time: 12:15:11 PM
 */
public class TestDiscoverSequenceVariantsIndelGenotypes extends TestFiles {


    private static final Log LOG = LogFactory.getLog(TestDiscoverSequenceVariantsIndelGenotypes.class);
    ...
    private static final String[] indelBasenames = new String[NUM_BASENAME_PER_GROUP * 2];
    ...
    private void configureTestGenome(DiscoverSequenceVariantsMode mode) {
        MutableString genomeString = new MutableString();
        String fragment = "CTGTAAAAAAAA";
        while (genomeString.length() < 10000) {
            genomeString.append(fragment);
        }
        RandomAccessSequenceTestSupport genomeForTest = new RandomAccessSequenceTestSupport(new String[]{genomeString.toString()});
    ...
    static int readIndexCursor = 0;

    static private int getNewReadIndex() {
        return readIndices[readIndexCursor++ % readIndices.length];
    }
